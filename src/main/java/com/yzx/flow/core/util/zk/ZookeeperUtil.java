...
public class ZookeeperUtil {
		...
		public void process(WatchedEvent event) {
			...
			if (event.getType() == EventType.NodeDataChanged) {
				try {
					...
					VersionUpdateExecute versionUpdateExecute = new VersionUpdateExecute(versionUpdateService,
							war_download_url, publickey, licence.getCustomerName(), licence.getPassword());
					if (event.getPath().equals(VersionUpdateExecute.parentNode + VersionUpdateExecute.COMPONENT)
							|| event.getPath()
									.equals(VersionUpdateExecute.parentNode + VersionUpdateExecute.orderNode)) {
						// 判断是否为当前组件升级
						byte[] data1 = zk.getData(VersionUpdateExecute.parentNode + VersionUpdateExecute.orderNode,
								dataWatcher, new Stat());
						byte[] data2 = zk.getData(VersionUpdateExecute.parentNode + VersionUpdateExecute.COMPONENT,
								dataWatcher, new Stat());
						String order = new String(data1, "utf-8");
						String status = new String(data2, "utf-8");// 之前存取的是否需要更新数据
					...
					// 监听到回滚
					if (event.getPath()
							.equals(VersionUpdateExecute.parentNode + VersionUpdateExecute.rollbackOrderNode)) {
						byte[] data3 = zk.getData(
								VersionUpdateExecute.parentNode + VersionUpdateExecute.rollbackOrderNode, dataWatcher,
								new Stat());
	...
	public void init() {
		try {
			...
			zk.exists(VersionUpdateExecute.parentNode + VersionUpdateExecute.orderNode, dataWatcher);
			zk.exists(VersionUpdateExecute.parentNode + VersionUpdateExecute.rollbackOrderNode, dataWatcher);
			zk.exists(VersionUpdateExecute.parentNode + VersionUpdateExecute.COMPONENT, dataWatcher);
			...
			if (licence != null) {
				...
				logger.info("licence认证通过");
			} else {
				logger.info("licence认证失败");
				// 退出系统
				System.exit(0);
			}
